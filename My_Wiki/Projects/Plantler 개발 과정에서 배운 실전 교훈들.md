---
id: f389419a
category: "[[10_Wiki/Projects]]"
confidence_score: 0.667
tags: [projects, 교훈, 실전, 개발철학, 품질관리]
last_reinforced: 2026-04-15
github_commit: "pending"
---

# [[Plantler 개발 과정에서 배운 실전 교훈들]]

## 📌 한 줄 통찰 (The Karpathy Summary)
> - `flutter analyze`로 **제로 에러 상태**를 목표로 유지

## 📖 구조화된 지식 (Synthesized Content)
- **추출된 패턴:**
  - `flutter analyze`로 **제로 에러 상태**를 목표로 유지
  - info, warning 레벨 이슈까지 모두 정리하면 코드 품질이 극적으로 향상됨
  - 정적 분석은 릴리즈 빌드 전 **필수 단계**
  - 공동 양육(Co-parenting)에서 식물 ID 공유 시 데이터 파편화 주의
  - AI 분석 결과, 사진, 일기가 모든 부모에게 동일하게 보여야 함

- **세부 내용:**
  - > "기능을 추가할 때마다 기존 기능이 깨지지 않는지 반드시 확인하라."
  - > "테스트 커버리지가 부족하면 결국 사용자가 QA를 해주게 된다."
  - > "복잡한 기능보다 안정적인 기능이 사용자에게 더 가치 있다."

## ⚠️ 모순 및 업데이트 (Contradictions & RL Update)
- **과거 데이터와의 충돌:** 현재까지 발견된 모순 없음.
- **정책 변화:** 이 문서를 통해 분류 정책에 변화 없음.

## 🔗 지식 연결 (Graph)
- **Parent:** [[Projects]]
- **Related:** [[Plantler 배포 워크플로우 — SCP 직접 전송 방식]], [[Plantler 백업복원 시스템 통합기]], [[Plantler AI 식물 분석 시스템]]
- **Raw Source:** [[00_Raw/10_개발_교훈_모음.md]]
