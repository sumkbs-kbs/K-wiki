---
id: 8795e144
category: "[[10_Wiki/Topics]]"
confidence_score: 0.4
tags: [topics, 프로젝트, ai학습, 다국어, 교육]
last_reinforced: 2026-04-15
github_commit: "pending"
---

# [[Ssak Language — AI 기반 다국어 학습 프로그램]]

## 📌 한 줄 통찰 (The Karpathy Summary)
> Ssak Language는 AI가 개인 맞춤형 외국어 학습 콘텐츠를 생성해주는 데스크탑 학습 도구다.

## 📖 구조화된 지식 (Synthesized Content)
- **추출된 패턴:**
  - **언어:** Python
  - **GUI:** CustomTkinter (프리미엄 다크 사이드바 레이아웃)
  - **AI 엔진:** Gemini API (1순위) / OpenAI (폴백) / Ollama (로컬)
  - **TTS:** gTTS (온라인) + pyttsx3 (오프라인)
  - **STT:** Speech-to-Text 엔진 (발음 연습용)

- **세부 내용:**
  - Ssak Language는 AI가 개인 맞춤형 외국어 학습 콘텐츠를 생성해주는 데스크탑 학습 도구다.
  - "학습지를 AI가 만들어준다"는 컨셉으로, 8개 언어를 지원한다.
  - 🇺🇸 영어 | 🇨🇳 중국어 | 🇯🇵 일본어 | 🇻🇳 베트남어
  - 🇫🇷 프랑스어 | 🇪🇸 스페인어 | 🇩🇪 독일어 | 🇰🇷 한국어
  - 1. 🏠 **홈** — 대시보드, 일일 목표, 스트릭 현황
  - 2. ✨ **AI 작문 학습지** — AI가 레벨별 작문 문제 생성
  - 3. 📝 **자유 작문** — 자유 주제 글쓰기 + AI 피드백
  - 4. 📙 **어휘 학습** — 주제별 단어장 생성

## ⚠️ 모순 및 업데이트 (Contradictions & RL Update)
- **과거 데이터와의 충돌:** 현재까지 발견된 모순 없음.
- **정책 변화:** 이 문서를 통해 분류 정책에 변화 없음.

## 🔗 지식 연결 (Graph)
- **Parent:** [[Topics]]
- **Related:** [[Ssak Image — 데스크탑 패키징과 Microsoft Store 배포]], [[Plantler 프로젝트 개요 — 식물 관리 AI 앱_1]], [[Plantler 프로젝트 개요 — 식물 관리 AI 앱_2]]
- **Raw Source:** [[00_Raw/22_SsakLanguage_프로젝트_개요.md]]
